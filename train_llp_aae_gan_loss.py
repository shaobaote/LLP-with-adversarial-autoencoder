...
def adjust_learning_rate(optimizer, epoch, learning_rate,batch_size):
    lr = learning_rate
    if epoch >= 500 and batch_size in [128,64]:
        lr = learning_rate * (0.5 ** (epoch // 100))  # i.e. 240,320
    elif epoch>=200 and batch_size in [16,32]:
...
def train_start(args):
    ...
    for epoch in range(n_epochs):
        ...
        for i, (imgs, labels) in enumerate(dataloader_train):
            ...
            #d_loss_cat=1*(-D_cat(real_cat)[1].mean() + D_cat(fake_cat)[1].mean()) + 10* gradient_penalty_cat
            #gradient_penalty_gauss = losses.cal_gradient_penalty(D_gauss, 'cuda', real_gauss, fake_gauss)
            #d_loss_gauss=1*(-D_gauss(real_gauss)[1].mean() + D_gauss(fake_gauss)[1].mean()) + 10* gradient_penalty_gauss
            #0.2,0.0001
            #GAN
            d_loss_gauss=-1*torch.mean(torch.log(D_gauss(real_gauss)[1] + eps)+torch.log(1-D_gauss(fake_gauss)[1] + eps))
            d_loss_cat=-1*torch.mean(torch.log(D_cat(real_cat)[1] + eps)+torch.log(1-D_cat(fake_cat)[1] + eps))
            ...
            #GAN
            g_loss_adv = -1*torch.mean(torch.log(D_gauss(fake_gauss)[1] + eps))-1*torch.mean(torch.log(D_cat(fake_cat)[1] + eps))      
        ...
        utils.plot_loss(method,sample,z_dim,dataset_name, epoch+1, batch_size, hy, xtick, y1,y2,y3,y4,y5,y6,y7)
