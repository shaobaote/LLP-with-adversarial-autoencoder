...
def train_start(args):
    ...
    optimizer_D_cat = Adam(D_cat.parameters(),lr=0.0003,betas=betas)
    ...
    for epoch in range(n_epochs):
        ...
        for i, (imgs, labels) in enumerate(dataloader_train):
            ...
            d_loss_cat=0.2*(-D_cat(real_cat)[1].mean() + D_cat(fake_cat)[1].mean()) + 0.0001* gradient_penalty_cat
            ...
            #WGAN-GP
            g_loss_adv=-0.02*(D_gauss(fake_gauss)[1].mean()+D_cat(fake_cat)[1].mean())
        ...
        #evaluate on test
        err_epoch_test,latent_z,real_labels,\
        entropy_instance_level_epoch_test,\
        ...
        #utils.plot_loss(method,dataset_name, epoch+1, batch_size, hy, xtick, y1,y2,y3,y4,y5,y6,y7)
    
        print('epoch={} error_test={}'.format(epoch,err_epoch_test))
        print('G_loss={} D_loss_cat={} D_loss_gauss={}'.format(G_loss[-1],D_cat_loss[-1],D_gauss_loss[-1]))
