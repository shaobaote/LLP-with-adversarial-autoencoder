...
def train_start(args):
    ...
    for epoch in range(n_epochs):
        ...
        for i, (imgs, labels) in enumerate(dataloader_train):
            ...
            t4=time.time()
            t_adv+=t4-t3
        time_recon.append(t_recon)
        time_adv.append(t_adv)
        plt.figure(figsize=(8,8))
        plt.plot(time_recon)
        plt.plot(time_adv)
        plt.savefig('./images/image/'+dataset_name+str(hy)+'time.png')
        np.save('./index/'+ dataset_name + '/'+'time_recon.npy',time_recon)
        np.save('./index/'+ dataset_name + '/'+'time_adv.npy',time_adv)
