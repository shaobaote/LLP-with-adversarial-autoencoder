...
def train_start(args):
    ...
    for epoch in range(n_epochs):
        ...
        for i, (imgs, labels) in enumerate(dataloader_train):
            ...
            if ent==2:
                real_prop=torch.Tensor([0.1 for i in range(10)]).cuda()
            else:
            ...
            d_loss_cat=1*(-D_cat(real_cat)[1].mean() + D_cat(fake_cat)[1].mean()) + 0.001* gradient_penalty_cat
            gradient_penalty_gauss = losses.cal_gradient_penalty(D_gauss, 'cuda', real_gauss, fake_gauss)
            d_loss_gauss=1*(-D_gauss(real_gauss)[1].mean() + D_gauss(fake_gauss)[1].mean()) + 0.001* gradient_penalty_gauss
            #0.2,0.0001
            #GAN
            ...
            if ent==2:
                ...
                g_loss_adv=-0.1*(D_gauss(fake_gauss)[1].mean())
            elif ent==3:
                g_loss_adv=-0.1*(D_cat(fake_cat)[1].mean())
        ...
        mean=round(np.mean(ent_list),2);std=round(np.std(ent_list),2);median=round(np.median(ent_list),2)
        ax=plt.gca()
        plt.text(1,1,va='top',ha='right',s='mean='+str(mean)+'\nstd='+str(std)+'\nmedian='+str(median),transform=ax.transAxes,fontsize=15)
        
        #plt.ylim(0, 40)
        plt.savefig('./images/'+ dataset_name +'/'+str(hy) +'_'+str(batch_size)+'_'+str(epoch)+method+sample+str(ent)+'.png')
        np.save('./index/'+ dataset_name +'/'+str(hy) +'_'+str(batch_size)+'_'+method+sample+str(ent)+' ent.npy',ent_list)
        plt.close()
        ...
        torch.save(En.state_dict(),'./state_dict/ent/'+ dataset_name + '/encoder '+str(hy)+' '+str(batch_size)+str(ent)+'.pth')
        torch.save(De.state_dict(),'./state_dict/ent/'+ dataset_name + '/decoder '+str(hy)+' '+str(batch_size)+str(ent)+'.pth')